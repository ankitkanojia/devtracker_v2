...
(function() {
    ...
    function q(a, f) {
        var n = function(e) { return new CKEDITOR.dom.element(e, a.document) },
            q = a.editable(),
            ...
            onOk: function() {
                ...
                if (c.info) {
                    ...
                    if (!this._.selectedElement)
                        for (var g = b.append(n("tbody")),
                            f = parseInt(c.txtRows, 10) || 0,
                            m = parseInt(c.txtCols, 10) || 0,
                            k = 0;
                            k < f;
                            k++) for (var h = g.append(n("tr")), l = 0; l < m; l++) h.append(n("td")).appendBogus();
                    f = c.selHeaders;
                    ...
                    if (null !== b.$.tHead && "row" != f && "both" != f) {
                        ...
                        for (l = g.getFirst(); 0 < h.getChildCount();) {
                            m = h.getFirst();
                            for (k = 0; k < m.getChildCount(); k++)
                                g = m.getChild(k), g.type == CKEDITOR.NODE_ELEMENT &&
                    ...
                    ("col" == f ||
                        ...
                        for (h = 0; h < b.$.rows.length; h++)
                            g = new CKEDITOR.dom.element(b.$.rows[h].cells[0]), g.renameNode("th"), g.setAttribute(
                                "scope",
                                "row");
                    if (this.hasColumnHeaders && "col" != f && "both" != f)
                        for (k = 0; k < b.$.rows.length; k++)
                            h = new CKEDITOR.dom.element(b.$.rows[k]), "tbody" == h.getParent().getName() &&
                            (g = new CKEDITOR.dom.element(h.$.cells[0]), g.renameNode("td"), g.removeAttribute(
                                "scope"));
                            ...
                            children: [
                                {
                                    ...
                                    children: [
                                        ...
                                        },
                                        {
                                            ...
                                            setup: function(e) { this.setValue(v(e)) },
                                            commit: l
    ...
    CKEDITOR.dialog.add("table", function(a) { return q(a, "table") });
