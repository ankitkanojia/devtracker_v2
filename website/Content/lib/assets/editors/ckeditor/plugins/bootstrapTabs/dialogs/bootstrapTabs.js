CKEDITOR.dialog.add("bootstrapTabsDialog",
    function(b) {
        return{
            ...
            contents: [
                {
                    ...
                    elements: [
                        { type: "html", html: b.lang.bootstrapTabs.infoHtml }, {
                            type: "text",
                            id: "tab-set-title",
                        ...
                        },
                        {
                            type: "html",
                            html:
                                '\x3chr style\x3d"display: block;position: relative;padding: 0;margin: 8px auto;width: 100%;max-height: 0;font-size: 1px;line-height: 0;clear: both;border: none;border-top: 1px solid #aaaaaa;border-bottom: 1px solid #ffffff;"/\x3e'
                        },
                        {
                            type: "select",
                            id: "tab-to-remove",
                            ...
                            items: [[b.lang.bootstrapTabs.removeTabDefault]],
                            setup: function(a) {
            ...
            onShow: function() {
                ...
                a &&
                (ascendant = a.getAscendant(function(a) {
                    return a instanceof CKEDITOR.dom.document ? !1 : a.hasClass("bootstrap-tabs")
                }));
            ...
            onOk: function() {
                ...
                if (this.insertMode) {
                    d = CKEDITOR.dom.element.createFromHtml(
                        '\x3cdiv class\x3d"bootstrap-tabs"\x3e\x3cul class\x3d"nav nav-tabs" role\x3d"tablist"\x3e\x3c!-- add tabs here --\x3e\x3c/ul\x3e\x3cdiv class\x3d"tab-content"\x3e\x3c!-- add tab panels here --\x3e\x3c/div\x3e\x3c/div\x3e');
                    for (c = 1; c <= a; c++) appendTabToElement(b, this, d, a, c);
                    b.insertElement(d)
                } else {
                    var d = this.element, f = d.find(".nav.nav-tabs li a.tab-link").count();
                    d.data("tab-set-title");
                    for (var k = d.findOne("ul.nav.nav-tabs"),
                        l = d.findOne("div.tab-content"),
                        h = k.find("li"),
                        m = l.find("div.tab-pane"),
                        c = 0;
                        c < h.count();
                        c++)
                        g === h.getItem(c).findOne("a.tab-link").getText() &&
                            h.count() === m.count() &&
                            (h.getItem(c).remove(), m.getItem(c).remove());
                    if (a > f) {
...
function appendTabToElement(b, a, e, g, d) {
    ...
    a = '\x3cdiv role\x3d"tabpanel" class\x3d"tab-pane" id\x3d"' +
        f +
        '"\x3e' +
        ('\x3cdiv class\x3d"tab-pane-content"\x3e' + b + " Content\x3c/div\x3e") +
        "\x3c/div\x3e";
    b = new CKEDITOR.dom.element.createFromHtml(
        '\x3cli role\x3d"presentation"\x3e\x3ca class\x3d"tab-link" href\x3d"#' +
        f +
        '" aria-controls\x3d"' +
        f +
        '" role\x3d"tab" data-toggle\x3d"tab"\x3e' +
        b +
        " Name\x3c/a\x3e\x3c/li\x3e");
